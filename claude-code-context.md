# 배경 설명 (VS Code Claude Code에게 전달용)

다른 Claude 대화창에서 이 프로젝트를 처음부터 같이 만들었습니다. 아래 내용을 읽고 현재 상태를 파악한 다음 작업을 이어가 주세요.

## 목표

회사 실무 개발 환경(PHP8 + Laravel, jQuery, Apache, MariaDB, Docker, GitLab)을 개인 로컬 환경에 그대로 재현하는 중입니다. 형상관리만 GitLab 대신 GitHub를 씁니다.
